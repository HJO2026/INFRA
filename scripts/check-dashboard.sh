...
# 판정: benchExpect=stub 인 패널의 쿼리가 하나라도 비면 실패. optional/later 는 보고만.
...
fails = 0; empties = {"stub": 0, "optional": 0, "later": 0}
for row_title, title, ref, exp, st, info, expr in rows:
    flag = ""
    if st in ("EMPTY", "ERROR", "NAN") and exp == "stub": fails += 1; flag = "  <-- FAIL"
...
print()
print(f"쿼리 {len(rows)}개: 비어 있음 stub={empties['stub']} optional={empties['optional']} later={empties['later']}")
sys.exit(1 if fails else 0)
PY
rc=$?
if [[ $rc -eq 0 ]]; then ok "대시보드: 스텁 환경 필수 패널 전부 데이터 있음"; else die "대시보드: 필수(stub) 패널 중 빈 것이 있다"; fi
