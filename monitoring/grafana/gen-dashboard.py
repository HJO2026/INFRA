...
각 패널의 benchExpect 필드(Grafana 는 무시): stub = 스텁 환경에서 반드시 값이 있어야 함, optional = 있으면 좋음(macOS 등에서 비어도 통과),
...
line(
    dict(title="요청 도착률 (k6 req/s, 시나리오별)", expect="stub", unit="reqps",
         targets=[(f'sum by (scenario) (rate(k6_http_reqs_total{RI}))', "{{scenario}}")],
         desc="k6 가 보낸 초당 요청 수. warmup 은 버리고 steady 만 집계한다"),
    dict(title="클라이언트 지연 p50/p95/p99 (k6, steady)", expect="stub", unit="ms",
...
line(
    dict(title="초당 성공 요청 수 (2xx/3xx, actuator 제외)", expect="stub", unit="reqps",
         targets=[(f'sum(rate({APP_REQ}_count{{status!~"5..",{NON_ACT}}}{RI}))', "성공 req/s"),
                  (f'sum by (uri) (rate({APP_REQ}_count{{status!~"5..",{NON_ACT}}}{RI}))', "{{uri}}")]),
    dict(title="서버 지연 p50/p95/p99 (histogram)", expect="stub", unit="s",
    ...
    dict(title="에러 비율: 5xx / 타임아웃 / 커넥션 거부", expect="stub", unit="percentunit", ymax=1,
...
row("포화 (Saturation)")
line(
    dict(title="HikariCP: active / pending / max", expect="stub", unit="short",
    ...
    dict(title="HikariCP 획득 대기 시간 (acquire p99 근사, max)", expect="stub", unit="s",
    ...
    dict(title="톰캣: busy / max 스레드, 대기 커넥션(큐 근사)", expect="stub", unit="short",
...
line(
    dict(title="PG 커넥션 상태별 (pg_stat_activity)", expect="stub", unit="short",
         targets=[(f'sum by (state) (pg_stat_activity_count{DB})', "{{state}}"),
                  ('max(pg_settings_max_connections)', "max_connections")]),
    dict(title="PG 대기 중 백엔드 (종류별)", expect="stub", unit="short",
    ...
    dict(title="JVM 스레드", expect="stub", unit="short",
...
line(
    ...
    dict(title="allocation rate · heap used", expect="stub", unit="Bps",
...
row("DB (PostgreSQL)")
line(
    dict(title="버퍼 캐시 히트율", expect="stub", unit="percentunit", ymax=1,
    ...
    dict(title="체크포인트 빈도 · 소요", expect="stub", unit="short",
    ...
    dict(title="WAL 생성량", expect="stub", unit="Bps",
...
line(
    dict(title="락 대기: 대기 백엔드 수 · 최장 대기(초) · 데드락", expect="stub", unit="short",
    ...
    dict(title="락 보유 (pg_locks, 모드별)", expect="stub", unit="short",
         targets=[(f'sum by (mode) (pg_locks_count{DB})', "{{mode}}")]),
    dict(title="트랜잭션 · 튜플", expect="stub", unit="short",
...
line(
    dict(title="DB 크기 · dead tuples", expect="stub", unit="bytes",
    ...
    dict(title="PG 디스크 블록 읽기 (캐시 미스)", expect="stub", unit="short",
...
line(
    dict(title="CPU (코어 사용량, 컨테이너별)", expect="stub", unit="short",
    ...
    dict(title="메모리 working set (컨테이너별) vs limit", expect="stub", unit="bytes",
